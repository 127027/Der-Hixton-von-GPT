...
These shared 3×80 values are historical pre-allocator research evidence and are **not the current product capitalization**. See `current-evidence.json` for the compact historical checkpoint and `../README.md` for the continuing learning journal; fresh 0.5.0 max-budget evidence is generated by the dashboard E2E.
...
Normal product UI/API/CLI expose only current V6. Backtests never activate Paper. Cloud/agent validation remains order-free. Local guarded Live execution is separately gated from the backtest product path.
