...
def test_v6_uses_owner_approved_repeat_capacity_without_changing_risk_rules() -> None:
    from hixton.domain.risk import DAILY_LOSS_LIMIT_PCT, MAX_DRAWDOWN_LIMIT_PCT

    assert V6_COIN_STRATEGY.slot_allocation == RANKED_REPEAT
    assert DAILY_LOSS_LIMIT_PCT == 5
    assert MAX_DRAWDOWN_LIMIT_PCT == 20
