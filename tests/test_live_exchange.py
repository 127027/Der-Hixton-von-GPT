...
def test_production_market_buy_uses_exact_persisted_250_quote_budget() -> None:
    live = LiveIntent(
        "production-two-slots",
        "account-fixture",
        "SOLUSDC",
        "BUY",
        "test-profile",
    ...
    transport.fill_rows[0]["qty"] = "2.5"
    transport.fill_rows[0]["quoteQty"] = "250"
    adapter = exchange(transport)
    result = adapter.submit(live)
    sent = transport.calls[0]
    assert sent[2]["quoteOrderQty"] == "250.00"
